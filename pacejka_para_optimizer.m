...
RMSE(1)=RMSE_calculator(para{1},data_SR,data_force); %Standard error of initial parameter value 
i=1; %step number
...
function RMSE=RMSE_calculator(Para,data_SR,data_force)
...
RMSE=sqrt(sum/(length(data_SR)-2));
end
...
for i=1:length(para)
    ...
    Lpara{i}=para;
    Lpara{i}(i)=para(i)-lr;
    dp(i)=(RMSE_calculator(Upara{i},data_SR,data_force)-RMSE_calculator(Lpara{i},data_SR,data_force))/2*lr;
end
end
